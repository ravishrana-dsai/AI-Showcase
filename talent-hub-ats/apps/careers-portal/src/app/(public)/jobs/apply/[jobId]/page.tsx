...
import { fetchJobApplyBundle, fetchJobMeta, parseQuestionOptionsJson } from "@/lib/hiring-portal";
...
export async function generateMetadata({ params }: ApplyPageProps) {
  ...
  return {
    title: meta ? `Apply: ${meta.title} | [Company] Careers` : "Apply | [Company] Careers",
  };
}
...
export default async function ApplyPage({ params }: ApplyPageProps) {
  ...
  const bundle = await fetchJobApplyBundle(jobId);
  if (!bundle) notFound();

  const { job, screeningQuestions: questionRows } = bundle;
  ...
  const descriptionIsHtml = job.description ? /<[a-z][\s\S]*>/i.test(job.description) : false;
  const descriptionLines =
    !descriptionIsHtml && job.description
